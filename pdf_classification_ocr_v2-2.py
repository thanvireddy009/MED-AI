...
ocr_df = df[df["OCR Applied"] == "Yes"][["File Name", "Pages", "OCR Text"]]

with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
    df.to_excel(writer, sheet_name="Classification", index=False)
    ocr_df.to_excel(writer, sheet_name="OCR Extracted Text", index=False)
    ...
    ocr_sheet = writer.sheets["OCR Extracted Text"]
    ocr_sheet.column_dimensions["A"].width = 30   # File Name
    ocr_sheet.column_dimensions["B"].width = 10   # Pages
    ocr_sheet.column_dimensions["C"].width = 100  # OCR Text

    wrap_alignment = Alignment(wrap_text=True, vertical="top")
    for row in ocr_sheet.iter_rows(min_row=2, max_row=ocr_sheet.max_row, min_col=3, max_col=3):
        for cell in row:
            cell.alignment = wrap_alignment
        # Give each row enough height to show the wrapped text
        ocr_sheet.row_dimensions[row[0].row].height = 300

    class_sheet = writer.sheets["Classification"]
    for col_letter, width in zip("ABCDEFGH", [28, 8, 22, 12, 12, 12, 12, 60]):
        class_sheet.column_dimensions[col_letter].width = width

print(f"\nExcel report saved: {output_file}")
print(f"  - Sheet 'Classification': all {len(df)} files")
print(f"  - Sheet 'OCR Extracted Text': {len(ocr_df)} files with OCR text")
